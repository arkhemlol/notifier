...
var schemaStatements = [...]string{
	...
	`CREATE TABLE plan_destinations (
		...
		plan_key INTEGER NOT NULL REFERENCES plans(plan_key) ON DELETE CASCADE,
	...
	`CREATE TABLE items (
		...
		plan_key INTEGER NOT NULL REFERENCES plans(plan_key) ON DELETE RESTRICT,
	...
	`CREATE TABLE deliveries (
		...
		item_key INTEGER NOT NULL REFERENCES items(item_key) ON DELETE RESTRICT,
		plan_destination_key INTEGER NOT NULL REFERENCES plan_destinations(plan_destination_key) ON DELETE RESTRICT,
	...
	`CREATE TABLE work_batch_items (
		batch_key INTEGER NOT NULL REFERENCES work_batches(batch_key) ON DELETE CASCADE,
		delivery_key INTEGER NOT NULL UNIQUE REFERENCES deliveries(delivery_key) ON DELETE RESTRICT,
		position INTEGER NOT NULL CHECK (position >= 0),
		PRIMARY KEY (batch_key, delivery_key),
		UNIQUE (batch_key, position)
	)`,
	`CREATE TABLE destination_status (
		plan_destination_key INTEGER PRIMARY KEY REFERENCES plan_destinations(plan_destination_key) ON DELETE CASCADE,
...
var indexStatements = [...]string{
	`CREATE INDEX idx_plan_destinations_ordered
		ON plan_destinations(plan_key, position)`,
	`CREATE INDEX idx_items_enqueued
		ON items(plan_key, enqueued_at, item_key)`,
	`CREATE INDEX idx_deliveries_pending_claim
		ON deliveries(plan_destination_key, state, item_key, delivery_key)`,
	`CREATE INDEX idx_deliveries_item_siblings
		ON deliveries(item_key, plan_destination_key, state)`,
	`CREATE INDEX idx_work_batches_retryable
		ON work_batches(plan_destination_key, state, retry_at, batch_key)`,
	`CREATE INDEX idx_work_batches_leased
		ON work_batches(plan_destination_key, state, lease_until, batch_key)`,
}
